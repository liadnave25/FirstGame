...
import com.example.oniongarlicrun.GameLogic
import com.example.oniongarlicrun.R
import com.example.oniongarlicrun.utils.Smanager

class MainActivity : AppCompatActivity() {

    private var cellMatrix: Array<Array<ImageView>>? = null
    private var bLeft: ImageButton? = null
    private var bRight: ImageButton? = null
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        gameLogic = GameLogic(
            context = this,
            cellMatrix = cellMatrix!!, // !! מבטיח שהערך לא יהיה null
    ...
    private fun drawEggplantAtLane(lane: Int) {
        for (i in 0 until numCols) {
            cellMatrix?.get(numRows - 1)?.get(i)?.setImageDrawable(null)
        }
        cellMatrix?.get(numRows - 1)?.get(lane)?.setImageResource(R.drawable.eggplant)
    }
